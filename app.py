...
option = st.sidebar.selectbox(
    '検索ワード　データ取得',
    ['API認証と検索ワードによるデータ取得', '検索ワードデータ分析']
)
...
if option == 'ユーザー名分析':
    ...
    if uploaded_file is not None:
        ...
        line = base.mark_line(color='pink').encode(y='ツイート数:Q', tooltip='ツイート数')
        ...
        hist2 = alt.Chart(wordCountMean, title='等級と文字数の関係性').mark_bar().encode(
            x='等級',
            y='平均文字数',
            tooltip=['平均文字数']
        )
...
if option == 'API認証と検索ワードによるデータ取得':
    ...
    if uploaded_file is not None:
        ...
        api = tweepy.API(auth)
        st.write('OAuth認証を完了しました。')
        search_word = st.text_input('検索ワードを入力してください。', 'min_faves:1000 髪')
        ...
        if st.button('データ取得'):
            ...
            columns = ['投稿日', 'ユーザー名', 'ツイート', 'いいね数', 'リツイート数']

            tweets = tweepy.Cursor(api.search_tweets, q=search_word, lang="ja").items(num_search_tweet)
            tweets = list(tweets)
                        
            tweet_data = []
            for tweet in tweets:
                tweet_data.append([tweet.created_at, tweet.user.name, tweet.text, tweet.favorite_count, tweet.retweet_count])
            ...
            st.download_button(
                label='CSVファイルをダウンロード',
                data=csv,
                file_name = 'search_tweets.csv',
                mime = 'text/csv'
            )
            st.dataframe(df)

if option == '検索ワードデータ分析':
    uploaded_file = st.file_uploader('分析用csvファイルをアップロード', type=['csv'])
    if uploaded_file is not None:
        df = pd.read_csv(uploaded_file)
        hist = alt.Chart(df, title='いいね数の傾向').mark_bar().encode(
